    ...
    window.crud = {
      ...
      dataTableConfiguration: {
		  'rowCallback': function(row, data, index){
			  const html=new DOMParser().parseFromString(data[5],"text/xml");
				
			if(html.querySelector('.read-status')){
				const readAt=html.querySelector('.read-status').getAttribute('data-value')
				if(!readAt){
					$(row).css('background-color', '#e7e69a');
				}
			}
        },
        ...
        responsive: {
            details: {
                ...
                renderer: function ( api, rowIdx, columns ) {
					console.log(api,rowIdx,columns,"dt");
